        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            <thead className="bg-zinc-50 dark:bg-zinc-900/50 border-b border-zinc-200 dark:border-zinc-800">
              <tr>
                <th className="px-4 py-3 font-mono text-xs text-zinc-500 font-medium uppercase w-12"></th>
                <th className="px-4 py-3 font-mono text-xs text-zinc-500 font-medium uppercase">
                  Project ID
                </th>
                <th className="px-4 py-3 font-mono text-xs text-zinc-500 font-medium uppercase">
                  Role
                ...
                <th className="px-4 py-3 font-mono text-xs text-zinc-500 font-medium uppercase text-right">
                  Latency
                </th>
              </tr>
            </thead>
              ...
              {PROJECTS.map((project) => (
                <React.Fragment key={project.id}>
                  <tr
                    ...
                    className={`
                      cursor-pointer transition-colors hover:bg-zinc-50 dark:hover:bg-zinc-900/30
                      ${activeProject === project.id ? 'bg-zinc-50 dark:bg-zinc-900/50' : ''}
                    `}
                  >
                    <td className="px-4 py-3 text-center">
                      <ChevronRight
                        className={`w-4 h-4 text-zinc-400 transition-transform duration-200 ${activeProject === project.id ? 'rotate-90 text-zinc-900 dark:text-zinc-100' : ''}`}
                      />
                    </td>
                    <td className="px-4 py-3 font-medium text-zinc-900 dark:text-zinc-100">
                      <div className="flex flex-col">
                        <span className="font-mono text-xs text-zinc-500 mb-0.5">{project.id}</span>
                    ...
                    <td className="px-4 py-3 text-right font-mono text-xs text-emerald-600 dark:text-emerald-500">
                      {project.latency}
                    </td>
                  </tr>
                  ...
                  {activeProject === project.id && (
                    <tr>
                      <td colSpan={5} className="p-0 bg-zinc-50/50 dark:bg-zinc-900/20">
